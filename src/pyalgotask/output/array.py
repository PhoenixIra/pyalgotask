...
import pyalgotask.output.pylatex_classes as clatex
...
class ArrayOutput(Output):
    ...
    def __init__(self, task_array, prefix, postfix, algorithm):
        ...
        self.phantom_length = (
            ...
            else max([len(str(x)) for x in self.task_array])
        )
        self.postfix = postfix
        self.algorithm = algorithm
        self.preamble = latex.Command("usetikzlibrary", "positioning")
        self.exercise_lengths_of_arrays = None
        self.exercise_left_labels = None
        self.exercise_top_labels = None
        self.exercise_phantom_length = None
    ...
    def init_exercise_output(
        ...
        """
        self.exercise_lengths_of_arrays = lengths_of_arrays
        self.exercise_left_labels = left_labels
        self.exercise_top_labels = top_labels
        self.exercise_phantom_length = phantom_length
        if self.phantom_length < self.exercise_phantom_length:
            self.phantom_length = self.exercise_phantom_length
    ...
    def generate_exercise_prefix(self, container):
        ...
        container.append(latex.NoEscape(self.prefix))
        ...
        container.append(latex.NoEscape(self.postfix))
    ...
    def generate_exercise_space(self, container):
        ...
        new_start_index = 0
        line_num = 0
        for array_length in self.exercise_lengths_of_arrays:
            if new_start_index == 0:
                self.empty_tikz_array(tikz=tikz, length=array_length)
                if self.exercise_left_labels:
                    self.tikz_array_left_label(
                        tikz, new_start_index, self.exercise_left_labels[line_num]
                    )
                if self.exercise_top_labels:
                    self.tikz_array_top_labels(
                        tikz, new_start_index, self.exercise_top_labels[line_num]
                    )
                ...
                first_options = (
                    ...
                    if not self.exercise_top_labels
    ...
    def generate_solution_space(self, container):
        ...
        new_start_index += len(self.task_array)
        ...
        for array, highlights in self.algorithm():
            ...
            if self.exercise_left_labels:
                self.tikz_array_left_label(
                    tikz, new_start_index, label=self.exercise_left_labels[line_num]
                )
            if self.exercise_top_labels:
                self.tikz_array_top_labels(
                    tikz, new_start_index, labels=self.exercise_top_labels[line_num]
                )
            last_start_index = new_start_index
        ...
        tikz.append(
            latex.TikZNode(
                ...
                text=clatex.StringEmptyContainer(
                    clatex.PhantomLength(self.phantom_length)
                ),
            )
        )
