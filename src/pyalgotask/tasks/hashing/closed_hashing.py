...
from pyalgotask.randomizer.parameters import ParameterRandomizer
...
class QuadraticProbingHashing(ProbingHashing):
    ...
    def init_hashing_argument_parser(self, parser):
        ...
        self.random_constant = ParameterRandomizer(parameter_name="constants")
            ...
            if not 0 < multiplication_constant <= 2 ** hashing_base.get_int_bits():
                raise ValueError(
                    (
                        ...
                        f"needs to be an integer in (0,2**{hashing_base.get_int_bits()}] but is {multiplication_constant}"
                    )
                )
